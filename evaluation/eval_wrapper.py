...
def create_data_pickles(
    data_root: str,
    cfg: Dict[str, Any],
) -> None:
    """
    Converts pre-processed dataset images into pickles.

    Parameters
    ----------
    data_root : str
        Pickled dataset root.
    cfg : Dict[str, Any]
        Model config.

    """
    if cfg.dataset != "Tusimple":
        raise NotImplementedError(f"Data serialization is implemented only for Tusimple dataset.")
    ...
    for data in dist_tqdm(loader):
        imgs, names = data
        image_name = names[0]
        root, dir, subdir, filename = image_name.split("/")
        pickle_name = Path(filename).with_suffix(".pickle")
        pickle_path = f"{root}-{dir}-{subdir}-{pickle_name}"
        pickle_path = Path(data_root) / pickle_path

        with pickle_path.open("wb") as result_file:
            img_data = imgs.cpu().detach().numpy()
            pickle.dump(img_data[0], result_file)


def run_test_tusimple_on_pickles(
    data_root: str,
    cfg: Dict[str, Any],
) -> None:
    """
    Runs testing on pickled inference results from TI device.

    Parameters
    ----------
    data_root : str
        Pickled results root.
    cfg : Dict[str, Any]
        Model config.
    ...
    for data in dist_tqdm(loader):
        ...
        pickle_name = filename.replace(".jpg", ".pickle")
        pickle_path = f"result_{root}-{dir}-{subdir}-{pickle_name}"
        pickle_path = Path(data_root) / pickle_path

        if not os.path.exists(pickle_path):
            raise RuntimeError(f"File {pickle_path} does not exist")

        with open(pickle_path.resolve(), "rb") as pickle_file:
            pred = pickle.load(pickle_file)

        pred = pred[0]  # numpy array from list
...
def eval_on_pickles(
    ...
    cfg: Dict[str, Any],
    ep: int = None,
    logger: DistSummaryWriter = None,
) -> None:
    """
    Main function for testing on pickled inference results from TI device.

    Parameters
    ----------
    pickles_path : str
        Root directory of pickled inference results from TI.
    cfg : Dict[str, Any]
        Model config.
    ep : int
        Epoch
    logger : DistSummaryWriter
       Logger

    """
    if cfg.dataset != "Tusimple":
        raise NotImplementedError("Validation on pickled data is not implemented!")
    exp_name = "tusimple_eval_tmp"
    run_test_tusimple_on_pickles(pickles_path, cfg)
